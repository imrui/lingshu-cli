...
test('init 创建零侵入项目结构（无 .lingshu / 无 package.json）', () => {
  ...
  // v0.4：CLAUDE.md 仅一行引入；规则正文（含生成头部）只在 AGENTS.md
  assert.equal(readFileSync(join(proj, 'CLAUDE.md'), 'utf8'), '@AGENTS.md\n', 'CLAUDE.md 应仅为一行 @AGENTS.md');
  const agents = readFileSync(join(proj, 'AGENTS.md'), 'utf8');
  assert.ok(!agents.includes('lingshu-template'), 'AGENTS.md 不应残留模板名');
  assert.match(agents, /generated by `lingshu sync`/, 'AGENTS.md 头部应引用 lingshu sync');
  assert.ok(!agents.includes('.lingshu/scripts'), 'AGENTS.md 不应再引用本地引擎脚本');
});
...
// ===== v0.4.0：CLAUDE.md 单行引入 + 肢体仓分发 =====

test('sync：CLAUDE.md 恒为一行 @AGENTS.md · 规则正文只在 AGENTS.md', () => {
  freshTmp();
  runCli(['init', 'pointer-proj', '--no-git', '--no-install-hooks']);
  const proj = join(TMP, 'pointer-proj');

  assert.equal(readFileSync(join(proj, 'CLAUDE.md'), 'utf8'), '@AGENTS.md\n', 'init 生成的 CLAUDE.md 应仅为一行引入');
  const agents = readFileSync(join(proj, 'AGENTS.md'), 'utf8');
  assert.match(agents, /架构核心准则/, 'AGENTS.md 应含规则正文');

  // 手改 CLAUDE.md → --check 判漂移 → sync 恢复为一行
  writeFileSync(join(proj, 'CLAUDE.md'), '# 手写规则\n', 'utf8');
  let r = runCli(['sync', '--check', '--baseline'], { cwd: proj });
  assert.equal(r.status, 1, '手改 CLAUDE.md 应被判漂移');
  assert.match(r.stdout, /CLAUDE\.md.*漂移/);
  r = runCli(['sync'], { cwd: proj });
  assert.equal(r.status, 0);
  assert.equal(readFileSync(join(proj, 'CLAUDE.md'), 'utf8'), '@AGENTS.md\n', 'sync 应恢复为一行引入');
});

test('sync：基线产物分发到肢体仓根目录，--check 一并校验', () => {
  freshTmp();
  runCli(['init', 'limb-dist', '--no-git', '--no-install-hooks']);
  const proj = join(TMP, 'limb-dist');
  runCli(['limb', 'init', 'app-server'], { cwd: proj });
  runCli(['limb', 'init', 'web-ui'], { cwd: proj });
  mkdirSync(join(proj, 'notes')); // 非肢体目录 · 不应分发

  let r = runCli(['sync'], { cwd: proj });
  assert.equal(r.status, 0);
  assert.match(r.stdout, /肢体仓 app-server\//, '输出应分组标注肢体仓');
  for (const limb of ['app-server', 'web-ui']) {
    assert.equal(readFileSync(join(proj, limb, 'CLAUDE.md'), 'utf8'), '@AGENTS.md\n', `${limb}/CLAUDE.md 应为一行引入`);
    assert.equal(
      readFileSync(join(proj, limb, 'AGENTS.md'), 'utf8'),
      readFileSync(join(proj, 'AGENTS.md'), 'utf8'),
      `${limb}/AGENTS.md 应与中枢产物一致`,
    );
  }
  assert.ok(!existsSync(join(proj, 'notes/AGENTS.md')), '非肢体目录不应分发');

  // 肢体产物漂移 → --check 失败并指出路径 → sync 修复
  writeFileSync(join(proj, 'app-server/AGENTS.md'), '# tampered\n', 'utf8');
  r = runCli(['sync', '--check'], { cwd: proj });
  assert.equal(r.status, 1, '肢体产物漂移应使 --check 失败');
  assert.match(r.stdout, /app-server\/AGENTS\.md.*漂移/);
  r = runCli(['sync'], { cwd: proj });
  assert.equal(r.status, 0);
  r = runCli(['sync', '--check'], { cwd: proj });
  assert.equal(r.status, 0, '修复后 --check 应通过');

  // personal 工具：--only 同样作用到肢体；之后 auto 按各目录"已存在"自行维护
  r = runCli(['sync', '--only=cursor'], { cwd: proj });
  assert.equal(r.status, 0);
  assert.ok(existsSync(join(proj, 'app-server/.cursor/rules/lingshu-core.mdc')), '--only 应作用到肢体仓');
});

test('sync --no-limbs：只写中枢，不碰肢体仓', () => {
  freshTmp();
  runCli(['init', 'no-limbs', '--no-git', '--no-install-hooks']);
  const proj = join(TMP, 'no-limbs');
  runCli(['limb', 'init', 'app-server'], { cwd: proj });

  const r = runCli(['sync', '--no-limbs'], { cwd: proj });
  assert.equal(r.status, 0);
  assert.ok(existsSync(join(proj, 'AGENTS.md')));
  assert.ok(!existsSync(join(proj, 'app-server/AGENTS.md')), '--no-limbs 不应分发到肢体仓');
  assert.ok(!existsSync(join(proj, 'app-server/CLAUDE.md')));
});

test('init --limbs 与 doctor：肢体仓克隆后即带基线产物，doctor 逐仓检查', () => {
  freshTmp();
  // 用本地裸仓模拟远程肢体仓
  const bare = join(TMP, 'remote-server.git');
  spawnSync('git', ['init', '--bare', '-q', bare], { encoding: 'utf8' });
  const r = runCli(['init', 'with-remote', `--limbs=my-server:${bare}`, '--no-git', '--no-install-hooks']);
  if (r.status !== 0) { console.error('STDOUT:', r.stdout); console.error('STDERR:', r.stderr); }
  assert.equal(r.status, 0);
  const proj = join(TMP, 'with-remote');
  assert.equal(readFileSync(join(proj, 'my-server/CLAUDE.md'), 'utf8'), '@AGENTS.md\n', 'init 应把基线产物分发进克隆的肢体仓');

  const d = runCli(['doctor'], { cwd: proj });
  assert.equal(d.status, 0);
  assert.match(d.stdout, /my-server\/AGENTS\.md/, 'doctor 应逐肢体仓检查基线产物');
});
